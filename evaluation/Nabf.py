...
def get_Nabf(I1, I2, f):
    Batch_size, Channel, m, n = I1.shape
    Nabf_values = []
    for b in range(Batch_size):
        for c in range(Channel):
            xrcw = f[b, c].astype(np.float64)
            x1 = I1[b, c].astype(np.float64)
            x2 = I2[b, c].astype(np.float64)
            gvA, ghA = sobel_fn(x1)
            ...
            NABF = compute_NABF(gA, gB, gF, gvA, ghA, gvB, ghB, gvF, ghF)
            Nabf_values.append(NABF)
    return np.mean(Nabf_values)

def compute_NABF(gA, gB, gF, gvA, ghA, gvB, ghB, gvF, ghF):
    Td = 2
    wt_min = 0.001
    P = 1
    Lg = 1.5
    ...
    ka = 22
    sigmaa = 0.5
    gAF = np.where((gA == 0) | (gF == 0), 0, np.where(gA > gF, gF / gA, gA / gF))
    gBF = np.where((gB == 0) | (gF == 0), 0, np.where(gB > gF, gF / gB, gB / gF))
    ...
    wtA = wt_min * np.ones_like(gA)
    wtB = wt_min * np.ones_like(gB)
    cA = np.ones_like(gA)
    cB = np.ones_like(gB)
